...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    fg[0] += weight_delta_dot * CppAD::pow(vars[delta_start + t + 1] - vars[delta_start + t], 2);
    fg[0] += weight_a_dot * CppAD::pow(vars[a_start + t + 1] - vars[a_start + t], 2);
    }
...
array<vector<double>, 6> MPC::PredictState_Kinematic(Eigen::VectorXd state, MPC mpc , Eigen::VectorXd coeffs, int N)
{
...

  {
            ...
            double p_x1 = p_x + cos(psi) * dt;
            double p_y1 = p_y + sin(psi) * dt;
            ...
            state << p_x1, p_y1, psi1, v1, cte1, epsi1;
            ...
            cout << i << "\t" <<  p_x1 << "\t"<< p_y1 << "\t"<< psi1 << "\t"<< v1 << "\t" << cte1 << "\t" << epsi1 << endl;
